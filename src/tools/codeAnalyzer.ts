import { CodeAnalysisResult, CodeIssue, QualityMetrics } from '../types/index.js';
 ...
 * 检测通用代码问题
 */
async function detectCommonIssues(code: string, language: string, issues: CodeIssue[]): Promise<void> {
  const lines = code.split('\n');
 ...
 */
async function analyzeJavaScript(code: string, issues: CodeIssue[], metrics: QualityMetrics): Promise<void> {
  const lines = code.split('\n');
 ...
 * Python 特定分析
 */
async function analyzePython(code: string, issues: CodeIssue[], metrics: QualityMetrics): Promise<void> {
  const lines = code.split('\n');
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmedLine.startsWith('import ') || trimmedLine.startsWith('from ')) {
      ...
      const hasNonImportCode = previousNonEmptyLines.some(l => 
        !l.trim().startsWith('#') && 
 ...
 * Java 特定分析
 */
async function analyzeJava(code: string, issues: CodeIssue[], metrics: QualityMetrics): Promise<void> {
  const lines = code.split('\n');
 ...
 * 通用分析
 */
async function analyzeGeneric(code: string, issues: CodeIssue[], metrics: QualityMetrics): Promise<void> {
  metrics.complexity = calculateCyclomaticComplexity(code);
}
 ...
 * 检测重复代码
 */
function detectDuplicateCode(lines: string[], issues: CodeIssue[]): void {
 ...
 * 计算可维护性指数
 */
function calculateMaintainabilityIndex(metrics: QualityMetrics, issueCount: number): number {
  const baseScore = 100;
 ...
 * 计算总体评分
 */
function calculateOverallScore(metrics: QualityMetrics, issues: CodeIssue[]): number {
 ...
 * 生成改进建议
 */
function generateRecommendations(issues: CodeIssue[], metrics: QualityMetrics): string[] {
