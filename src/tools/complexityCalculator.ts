import { ComplexityResult, ComplexityMetrics } from '../types/index.js';
 ...
 * 计算基础指标
 */
function calculateBasicMetrics(code: string, metrics: ComplexityMetrics): void {
 ...
 * 计算Halstead复杂度
 */
function calculateHalsteadComplexity(code: string, language: string): any {
 ...
 * 计算可维护性指数
 */
function calculateMaintainabilityIndex(metrics: ComplexityMetrics): number {
 ...
 * 分析复杂度
 */
function analyzeComplexity(metrics: ComplexityMetrics): string {
 ...
 * 生成复杂度改进建议
 */
function generateComplexityRecommendations(metrics: ComplexityMetrics): string[] {
 ...
 * 评估风险等级
 */
function assessRiskLevel(metrics: ComplexityMetrics): 'low' | 'medium' | 'high' | 'critical' {
  let riskScore = 0;
  
  // 圈复杂度风险
  if (metrics.cyclomaticComplexity > 20) riskScore += 3;
  else if (metrics.cyclomaticComplexity > 10) riskScore += 1;
  
  // 认知复杂度风险
  if (metrics.cognitiveComplexity > 25) riskScore += 3;
  else if (metrics.cognitiveComplexity > 15) riskScore += 1;
  
  // 可维护性指数风险
  if (metrics.maintainabilityIndex < 40) riskScore += 3;
  else if (metrics.maintainabilityIndex < 60) riskScore += 1;
  
  // Halstead复杂度风险
  if (metrics.halsteadComplexity.difficulty > 30) riskScore += 2;
  if (metrics.halsteadComplexity.bugs > 0.2) riskScore += 2;
...
function getControlStructures(language: string): string[] {
  return ['if', 'else', 'while', 'for', 'switch', 'try', 'catch', 'finally'];
}

function isInCondition(line: string): boolean {
  return /\b(if|while|for)\s*\(/.test(line) || /\?.*:/.test(line);
}

function hasRecursiveCall(line: string, code: string): boolean {
  // 简化的递归检测
...
function getOperators(language: string): string[] {
  const common = ['+', '-', '*', '/', '%', '=', '==', '!=', '<', '>', '<=', '>=', '&&', '||', '!'];
...
function getOperands(code: string, language: string): string[] {
  // 简化的操作数提取
...
function countTokens(code: string, tokens: string[]): Record<string, number> {
